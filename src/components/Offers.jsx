  ...
  return <section className="bg-slate-50/70">
      <div className="mx-auto max-w-[1440px] px-4 py-12 sm:px-8 lg:py-16">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
          {OFFERS.map((o, i) => <Reveal key={o.name} delay={i * 0.07}>
              <div className="card card-hover flex h-full flex-col p-4">
                <div className="flex items-start justify-between">
                  {o.brand === "googleads" ? <GoogleAdsLogo size={44} /> : o.brand === "meta" ? <MetaLogo size={44} /> : o.brand === "semrush" ? <SemrushLogo size={44} /> : o.brand === "canva" ? <CanvaLogo size={44} /> : o.brand === "clickfunnels" ? <ClickFunnelsLogo size={44} /> : <LogoBadge brand={BRANDS[o.brand]} size={44} />}
                  {o.sponsored && <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-600 ring-1 ring-amber-200">
                      Sponsored
                    </span>}
                </div>
...
function AffiliatePrograms() {
  return <section className="mx-auto max-w-[1440px] px-4 py-12 sm:px-8 lg:py-16">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
        {PROGRAMS.map((p, i) => <Reveal key={p.name} delay={i * 0.07}>
            <div className="card card-hover flex h-full flex-col p-4">
              <div className="flex items-center gap-3">
                {p.brand === "amazon" ? <AmazonLogo size={44} /> : p.brand === "clickbank" ? <ClickBankLogo size={44} /> : p.brand === "impact" ? <ImpactLogo size={44} /> : p.brand === "shareasale" ? <ShareASaleLogo size={44} /> : p.brand === "cj" ? <CJLogo size={44} /> : <LogoBadge brand={BRANDS[p.brand]} size={44} />}
