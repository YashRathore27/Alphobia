...
function DealCard({ deal }) {
  ...
  return <div className="card card-hover flex h-full flex-col p-4">
      <div className="flex items-center gap-3">
        {deal.brand === "canva" ? <CanvaLogo size={42} /> : deal.brand === "hostinger" ? <HostingerLogo size={42} /> : deal.brand === "semrush" ? <SemrushLogo size={42} /> : deal.brand === "envato" ? <EnvatoLogo size={42} /> : deal.brand === "clickfunnels" ? <ClickFunnelsLogo size={42} /> : <LogoBadge brand={BRANDS[deal.brand]} size={42} />}
        <div>
  ...
  return <section className="bg-slate-50/70">
      <div className="mx-auto max-w-[1440px] px-4 py-12 sm:px-8 lg:py-16">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
          {DEALS.map((d, i) => <Reveal key={d.name} delay={i * 0.07}>
              <DealCard deal={d} />
            </Reveal>)}
        </div>
      </div>
          ...
          <div className="relative flex flex-col items-center justify-center gap-8 lg:flex-row lg:gap-6">
            {COMPARE.map((c, i) => <div key={c.name} className="flex flex-col items-center gap-8 lg:flex-row lg:gap-6">
                ...
                <div className="flex w-64 items-center gap-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition-shadow hover:shadow-lg">
                  {c.brand === "googleads" ? <GoogleAdsLogo size={50} /> : <LogoBadge brand={BRANDS[c.brand]} size={50} />}
                  <div>
