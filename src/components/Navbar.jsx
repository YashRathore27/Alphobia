...
function Navbar() {
    ...
    /* Center nav */
        ...
        <nav className="mx-4 hidden items-center gap-1 lg:flex">
          {NAV_LINKS.map((l) => <button
            key={l.label}
            onClick={() => go(l.route)}
            className={`flex items-center gap-1 rounded-xl px-3 py-2 text-sm font-medium transition-colors ${route === l.route ? "bg-blue-50 text-blue-600" : "text-slate-600 hover:bg-blue-50 hover:text-blue-600"}`}
          >
            {l.label}
          </button>)}
        </nav>
  ...
  />
            <button
    onClick={() => go("search")}
            className="gradient-blue absolute right-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-xl text-white transition-transform hover:scale-105"
    aria-label="Search"
  >
  ...
  >
    ...
    placeholder="Search for tools, deals or offers..."
    onKeyDown={(e) => e.key === "Enter" && go("search")}
    className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-4 pr-11 text-sm outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-100"
  />
          ...
          {NAV_LINKS.map((l) => <button
            key={l.label}
            onClick={() => go(l.route)}
            className={`flex w-full items-center justify-between rounded-xl px-3 py-3 text-sm font-medium ${route === l.route ? "bg-blue-50 text-blue-600" : "text-slate-700 hover:bg-blue-50"}`}
          >
            {l.label}
          </button>)}
        </div>}
