...
function HeroSection({ tool }) {
  ...
  return <div className="grid gap-8 lg:grid-cols-[70%_30%]">
      {
    /* LEFT — Product Info */
  }
...
function GallerySection({ tool }) {
  ...
  >
        ...
        <div className="flex flex-col gap-4 lg:flex-col">
          {(tool.gallery ?? []).slice(0, 2).map((src, i) => <div
    key={i}
  ...
  >
              ...
              {i === 1 && (tool.gallery?.length ?? 0) > 2 && <div className="absolute inset-0 flex items-center justify-center bg-black/40">
    ...
    /* Lightbox */
      ...
      {viewerSrc && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={() => setViewerSrc(null)}>
          <img src={viewerSrc} alt="" className="max-h-[90vh] max-w-full rounded-2xl object-contain shadow-2xl" onClick={(e) => e.stopPropagation()} />
        </div>}
    </>;
}
function RelatedToolRow({ name, desc, brandKey }) {
  return <button
...
function InformationSection({ tool }) {
  const relatedTools = [
  ...
  return <div className="grid gap-8 lg:grid-cols-[30%_70%]">
      {
    /* LEFT — Sidebar */
  }
    ...
    /* Related Tools */
        ...
        <div className="rounded-[20px] bg-white p-5 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
          ...
          <div className="space-y-0.5">
            {relatedTools.map((rt) => <RelatedToolRow key={rt.brandKey} {...rt} brandKey={rt.brandKey} />)}
          </div>
          <button
  ...
  >
        ...
        <div className="relative overflow-hidden rounded-[20px] bg-gradient-to-br from-[#6A5AF9] to-[#4C3DF2] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
          ...
          <div className="relative z-10">
            ...
            <div className="mt-4 flex items-center gap-1 text-amber-300">
              {[1, 2, 3, 4, 5].map((i) => <Star key={i} className={`h-4 w-4 ${i <= Math.round(tool.rating) ? "fill-amber-300" : "fill-white/20"}`} />)}
    ...
    /* Overview */
        ...
        <div>
          ...
          {tool.overview?.map((p, i) => <p key={i} className={`text-[17px] leading-relaxed text-[#666666] ${i > 0 ? "mt-[18px]" : "mt-4"}`}>{p}</p>)}
        </div>
    ...
    /* Strengths */
          ...
          <div className="rounded-[20px] bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
            ...
            <ul className="mt-3 space-y-3">
              {tool.strengths.map((s) => <li key={s} className="flex items-start gap-2.5 text-[14px] text-[#666666]">
    ...
    /* Considerations */
          ...
          <div className="rounded-[20px] bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
            ...
            <ul className="mt-3 space-y-3">
              {tool.considerations.map((c) => <li key={c} className="flex items-start gap-2.5 text-[14px] text-[#666666]">
...
function PlanComparisonSection({ plans }) {
  const rows = [
  ...
  return <div>
      ...
      <div className="mt-6 overflow-x-auto rounded-[20px] bg-white shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
        <table className="w-full min-w-[500px]">
          ...
          <tbody>
            {rows.map((row, ri) => <tr key={row.feature} className={ri % 2 === 0 ? "bg-[#F7F7FC]" : ""}>
                <td className="p-5 text-[14px] font-semibold text-[#111111]">{row.feature}</td>
                {row.values.map((v, vi) => <td key={vi} className="p-5 text-center text-[14px] text-[#666666]">
                    {v === "\u2713" ? <Check className="mx-auto h-5 w-5 text-[#3BB273]" /> : v === "\u2717" ? <X className="mx-auto h-5 w-5 text-[#F04C4C]" /> : v}
                  </td>)}
              </tr>)}
          </tbody>
        </table>
...
function AffiliateSection() {
  ...
  return <div className="rounded-[20px] bg-gradient-to-br from-[#F7F7FC] to-white p-8 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
...
function ToolDetail({ id }) {
  const tool = id ? TOOLS_DATA[id] : void 0;
  if (!tool) {
  ...
  return <div className="bg-[#F7F7FC] pb-16">
    ...
    /* Reviews */
        ...
        <div>
          ...
          <div className="mt-6 space-y-4">
            {tool.reviews.map((rv) => <div key={rv.n} className="rounded-[20px] bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
