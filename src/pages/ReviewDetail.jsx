...
function ReviewDetail({ id }) {
  ...
  const review = id ? REVIEWS_DATA[id] : void 0;
  if (!review) {
  ...
  const scrollTo = (id2) => {
    ...
    document.getElementById(`section-${id2}`)?.scrollIntoView({ behavior: "smooth" });
  };
  return <div className="bg-bg-primary pb-16">
      <PageHeader
    crumbs={[
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="inline-flex items-center gap-1 text-[14px] font-bold text-amber-500">
                    <span className="inline-flex">
                      {[1, 2, 3, 4, 5].map((i) => <span key={i} className="text-amber-400">{i <= Math.round(review.rating) ? "\u2605" : "\u2606"}</span>)}
                    </span>
                    {review.rating}
                  </span>
                </div>
          ...
          <Reveal>
            <div>
              ...
              <div className="mt-4 space-y-4">
                {review.overview.map((p, i) => <p key={i} className="text-[16px] leading-[1.8] text-slate-500 sm:text-[17px]">{p}</p>)}
              </div>
            </div>
          </Reveal>
          </div>
              ...
              <div className="mt-4 grid gap-6 sm:grid-cols-2">
                <div className="card border-emerald-100 p-6">
                  ...
                  <ul className="space-y-3">
                    {review.pros.map((p) => <li key={p} className="flex items-start gap-3 text-[14px] text-slate-600">
                ...
                <div className="card border-rose-100 p-6">
                  ...
                  <ul className="space-y-3">
                    {review.cons.map((c) => <li key={c} className="flex items-start gap-3 text-[14px] text-slate-600">
  ...
  ].map((f) => <div key={f.title} className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <div className="card border-purple-200 bg-gradient-to-b from-purple-50/60 to-white p-5 ring-1 ring-purple-100">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="mt-1 inline-flex">
                  {[1, 2, 3, 4, 5].map((i) => <span key={i} className={`text-[18px] ${i <= Math.round(review.score) ? "text-amber-400" : "text-slate-200"}`}>★</span>)}
                </div>
