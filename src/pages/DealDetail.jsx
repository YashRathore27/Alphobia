...
function MiniDashboard() {
  return <div className="card overflow-hidden rounded-2xl border-slate-200/70">
...
function DealDetail({ id }) {
  ...
  const deal = id ? DEALS_DATA[id] : void 0;
  if (!deal) {
  ...
  const trustItems = deal.trustIcons.map((name) => ({
    ...
    label: name === "BadgeCheck" ? "Official Partner" : name === "ShieldCheck" ? "Verified Offer" : name === "Wallet" ? "No Hidden Charges" : name === "Rocket" ? "Instant Activation" : name === "Sparkles" ? "Beginner Friendly" : "Global Access"
  }));
  ...
  />
              <div className="pt-6">
                {tab === "Overview" && <div className="grid gap-8 lg:grid-cols-2">
                    <div>
                      ...
                      <ul className="mt-3 space-y-2.5">
                        {deal.checklist.map((item) => <CheckItem key={item}>{item}</CheckItem>)}
                      </ul>
                    </div>
                    <MiniDashboard />
                  </div>}
                {tab === "Benefits" && <ProsCons pros={deal.pros} cons={deal.cons} />}
                {tab === "Screenshots" && <div className="grid gap-6 sm:grid-cols-2">
                    <MiniDashboard />
                    <MiniDashboard />
                  </div>}
                {tab === "Reviews" && <div className="space-y-4">
                    {deal.reviews.map((rv) => <div key={rv.n} className="rounded-2xl border border-slate-100 bg-slate-50/60 p-5">
          ...
          <Reveal>
            <div>
              ...
              <div className="grid grid-cols-2 gap-6 xl:grid-cols-4">
                {deal.similar.map((s) => <div key={s.name} className="card card-hover flex flex-col items-center p-4 text-center">
