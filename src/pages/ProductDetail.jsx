...
function ProductDetail({ id }) {
  ...
  const product = id ? PRODUCTS_DATA[id] : void 0;
  if (!product) {
  ...
  return <div className="bg-[#F8FAFC] pb-8">
      <PageHeader
    crumbs={[
  ...
  >
    ...
    /* Right — Quick Info Cards */
              ...
              <div className="grid grid-cols-2 gap-3">
                {product.meta.map((m) => <div key={m.label} className="flex flex-col items-center justify-center rounded-xl border border-[#EEF2F7] bg-white p-4 text-center shadow-[0_8px_24px_rgba(0,0,0,0.05)]">
          ...
          <Reveal>
            <div className="mt-12 grid items-start gap-8 lg:grid-cols-[55%_45%]">
              <div>
                ...
                {product.overview?.length ? product.overview.map((p, i) => <p key={i} className="mt-4 text-[15px] leading-relaxed text-[#6B7280]">{p}</p>) : <>
  ...
  />
                {product.gallery && product.gallery.length > 0 && <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none">
                    {product.gallery.map((src, i) => <img
    key={i}
    src={src}
    alt=""
  ...
  >
          ...
          <Reveal>
            <div className="mt-14">
              ...
              <div className="mt-6 space-y-5">
                {product.reviews.map((rv) => <div key={rv.n} className="rounded-[20px] border border-[#EEF2F7] bg-white p-6 shadow-[0_8px_24px_rgba(0,0,0,0.05)]">
    ...
    /* ─────── ALTERNATIVES ─────── */
        ...
        <section id="alternatives" ref={(el) => {
    sectionRefs.current.alternatives = el;
  }} className="scroll-mt-28">
          <Reveal>
            <div className="mt-14">
              <h2 className="text-[24px] font-bold text-[#111827]">Alternatives</h2>
              <div className="mt-6 grid grid-cols-2 gap-5 md:grid-cols-4">
                {product.alternatives.map((a) => <div
    key={a.name}
