...
function useCountdown(initial) {
  ...
  useEffect(() => {
    const id = setInterval(() => setLeft((s2) => s2 > 0 ? s2 - 1 : 24 * 3600), 1e3);
...
function EndingSoonCard({ deal }) {
  ...
  return <div className="flex flex-col overflow-hidden rounded-[18px] bg-white shadow-[0_10px_30px_rgba(0,0,0,0.06)] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
...
function RecentlyAddedCard({ deal }) {
  return <div className="flex h-[160px] w-[240px] shrink-0 flex-col overflow-hidden rounded-[16px] bg-white shadow-[0_10px_30px_rgba(0,0,0,0.06)] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
...
function DealListing() {
  ...
  return <div className="bg-[#F8F8FC] pb-16">
  ...
  >
          ...
          <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {ENDING_SOON.map((d) => <EndingSoonCard key={d.key} deal={d} />)}
          </div>
        </section>
    ...
    /* ─────────── SECTION 4 — Recently Added ─────────── */
        ...
        <section className="mx-auto w-full max-w-[1320px] px-4 sm:px-6">
          ...
          <div ref={scrollRef} className="mt-6 flex gap-5 overflow-x-auto pb-2 scrollbar-none">
            {RECENTLY_ADDED.map((d) => <RecentlyAddedCard key={d.key} deal={d} />)}
          </div>
        </section>
