...
function AllCategories() {
  return <div className="bg-[#F8FAFC] pb-4">
    ...
    return <Reveal key={c.id} delay={i % 5 * 0.07}>
                <button
    ...
    >
                    {c.image ? <img src={c.image} alt={c.title} className="h-full w-full object-contain" /> : <Icon className="h-7 w-7" />}
                  </span>
...
function CategoryListing({ id }) {
  const [sort, setSort] = useState("Popular");
  const [showFilters, setShowFilters] = useState(false);
  const [query, setQuery] = useState("");
  const cat = id ? getCategory(id) : void 0;
  ...
  >
            {["Popular", "Highest Rated", "Price: Low to High", "Newest"].map((o) => <option key={o}>{o}</option>)}
          </select>
        </div>
      </div>
    ...
    /* Listing Area */
        ...
        <div>
          ...
          <div className="space-y-5">
            {filtered.map((t, i) => <Reveal key={t.name} delay={i * 0.05}>
                <ToolRowCard t={t} />
              </Reveal>)}
          </div>
